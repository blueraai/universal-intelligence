...
def signal_handler(sig, frame):
    global should_exit
    logger.info("Received shutdown signal, completing tests...")
    should_exit = True
...
async def diagnostic_client() -> None:
    ...
    while time.time() - start_time < args.duration and not should_exit:
        if not connection_alive:
            try:
                ...
                async with websockets.connect(
                    ...
                    while (
                        time.time() - start_time < args.duration and 
                        ...
                        if current_time - code_time >= 30:  # Every 30 seconds
...
def print_status(final=False) -> None:
    ...
    if final:
        ...
        print("\nNEXT STEPS:")
        print("1. Run server with increased logging: set logging level to DEBUG")
        print("2. Open browser console to monitor WebSocket traffic")
        print("3. Consider testing with alternative browsers")
        print("4. Check for network issues using ping and traceroute")
